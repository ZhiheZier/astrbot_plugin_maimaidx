...
def draw_footer_text(
    ...
    primary_font = ImageFont.truetype(str(FOTNEWRODIN), size)
    fallback_font = ImageFont.truetype(str(SIYUAN), size)
    parts: List[Tuple[str, ImageFont.FreeTypeFont]] = []

    for char in text:
        font = primary_font if char.isascii() else fallback_font
        if parts and parts[-1][1] is font:
            parts[-1] = (parts[-1][0] + char, font)
        else:
            parts.append((char, font))

    total_width = sum(dr.textlength(part, font=font) for part, font in parts)
    ...
    for part, font in parts:
        ...
        x += dr.textlength(part, font=font)
...
async def draw_music_info(
    ...
    draw_footer_text(
        ...
        default_color, 3, (255, 255, 255, 255),
    )
...
async def draw_music_banquet_info(music: Music) -> MessageSegment:
    ...
    draw_footer_text(
        dr, 600, 1100, 25,
        f'Designed by Yuri-YuzuChaN & BlueDeer233. Generated by {get_botname()} BOT',
        stroke_color, 3, (255, 255, 255, 255),
    )
    return MessageSegment.image(image_to_base64(im))
